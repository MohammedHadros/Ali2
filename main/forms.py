...
class RequestForm(forms.ModelForm):
    ...
    def save(self, request, slug, commit=True):
        ...
        request.save()
        message=" لديك طلب جديد من "+ str(request.customer) + " يريد حجز غرفة في فندق " + request.hotel.hotel_name
        notification=BroadcastNotification2(message=message ,broadcast_on=datetime.now(), request=request,reciver=request.hotel.user )
        notification.save()
        room_name=request.hotel.user.get_username()
