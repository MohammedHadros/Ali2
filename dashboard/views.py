...
from django.http import HttpResponseRedirect,HttpResponse
from datetime import datetime
from dashboard.models import Booking, Hotel, Season, Request,BroadcastNotification2
...
# Customize this function to fetch and format notifications as needed
    noftification_db=BroadcastNotification2.objects.filter(reciver=request.user)
    notifications = [
    ...
    data = [{"message": notification.message,
              "broadcast_on": notification.broadcast_on,
              "slug":notification.request.hotel.slug
              } for notification in noftification_db]
    return JsonResponse({'notifications': data})


# def get_customer_notifications(request):
# # Customize this function to fetch and format notifications as needed
#     noftification_db=BroadcastNotification2.objects.all()
#     print(request.user)
...
def calculate_hours(dt):
    return (datetime.now()-dt)/(1000*3600)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.status=='pending':
                messagea=" طلبك قيد التنفيذ الخاص بالفندق"    + request.hotel.hotel_name
            elif request.status=='accepted':
                messagea=" لقد تم قبول طلبك الخاص بالفندق "+ request.hotel.hotel_name
            elif request.status=='rejected':
                messagea=" لقد تم رفض طلبك الخاص بالفندق "+ request.hotel.hotel_name
            else:
                messagea=" راقب طلباتك " + request.hotel.hotel_name
            request.save()
            notification=BroadcastNotification2(message=messagea ,broadcast_on=datetime.now(), request=request,reciver=request.hotel.user )
            notification.save()
            room_name=request.customer.user.get_username()
